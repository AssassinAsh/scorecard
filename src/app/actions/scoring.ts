...
export async function startNewOver(inningsId: string, overNumber: number, bowlerName: string) {
  const supabase = await createClient()
...
export async function recordBall(ballData: CreateBallForm) {
  ...
  // Calculate updates
  const totalRuns = calculateBallRuns(ballData.runs_off_bat, ballData.extras_runs)
  ...
  const newOversCompleted = calculateOvers(totalLegalBalls)
